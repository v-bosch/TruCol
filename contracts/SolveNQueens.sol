...
contract SolveNQueens {
    ...
    function main(bool[4][4] memory unsolved_board) public returns (bool[4][4] memory solved_board) {
        (bool a, bool[4][4] memory solution) = solveNQ(unsolved_board, N);
        return(solution);
    ...
    function solveNQ(bool[4][4] memory board_in, uint8 col) public returns (bool, bool[4][4] memory) { 
        ...
        for (uint8 i = 0; i < N; i++) { 
            ...
            if (isSafe(board_in, i, col)) { 
                ...
                (bool a, bool[4][4] memory c) = solveNQ(board_in, col + 1);
                if (a)
                    return (true, c); 
    ...
    function isSafe(bool[4][4] memory b, uint8 row, uint8 col) public view returns (bool) { 
        uint8 i = row;
        uint8 j = col; 
